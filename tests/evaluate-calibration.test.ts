 ...
 *  - that the ORDINARY persona-research prompt stays byte-for-byte locked
 *    when `calibration` is not set. The golden includes issue #39's deliberate
 *    position-shift addition; unrelated calibration framing must still never
...
describe('buildEvaluationPrompt — ordinary persona-research prompt stays isolated from calibration framing (issue #35)', () => {
  it('matches the reviewed persona golden, including issue #39 position-shift diagnostics, when calibration is not set', () => {
    const questionnaireId = randomUUID()
    const q2 = randomUUID()
    const p1 = randomUUID()
